  ...
  <!-- CONTAINER -->
		...
		<table width="700" border="1" class="listTable">
			<tr height = "40" align="right">
				<td colspan="5" class="td_title">
					<p>Q&A 게시판
						<Button onclick="location.href ='${ctx}/QNAWriteController.do'">글쓰기</Button>
					</p>
				</td>
			</tr>
	...
	<c:if test="${count>0}">
		<c:if test="${blockStart > page}">
			<a href="${ctx}/QNAListController.do?pageNum=${blockStart-page}" class="pagingBA">[이전]</a>
		</c:if>
		...
		<c:if test="${blockEnd < pageCount}">
			<a href="${ctx}/QNAListController.do?pageNum=${blockStart+page}" class="pagingBA">[다음]</a>
		</c:if>
	</c:if>
	</div>
	</div>
